...
function loadConfig(directory: string, worktree: string): NoteLoggerConfig {
  // Read global config from its known absolute path (~/.config/opencode/opencode.json).
  // NOTE: `directory` is the current working directory (same as worktree), NOT the
  // global config dir — confirmed from OpenCode plugin docs. We must use homedir()
  // to find the global config reliably.
  const globalConfigPath = resolve(homedir(), ".config/opencode/opencode.json")
  const globalRaw = readPluginConfig(globalConfigPath)

  // Read project config from working directory (may only have project + vault)
  const projectRaw = readPluginConfig(resolve(directory, "opencode.json"))

  // Merge: project-level values override global, but only if explicitly set (not null/undefined)
  // This lets project configs set just "project" and "vault" while inheriting
  // model/base_url/thresholds/etc from the global config.
...
export const ObsidianNoteLoggerPlugin = async (
  ...
  await client.app.log({
    body: {
      ...
      level: "info",
      message: `obsidian_note_logger loaded — project=${config.project} vault=${config.vault} min_tools=${config.min_tool_calls} min_msgs=${config.min_messages}`,
    },
  })
